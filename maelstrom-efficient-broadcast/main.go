...
func (s *Server) HandleTopology(msg maelstrom.Message) error {
	...
	// ---- IGNORE maelstrom suggested topology, and use a master-slave based topology
	if s.Node.ID() == MASTER_NODE_ID {
		for _, id := range s.Node.NodeIDs() {
			if id != s.Node.ID() {
				s.NeighbouringNodes = append(s.NeighbouringNodes, id)
			}
		}
	} else {
		s.NeighbouringNodes = []string{MASTER_NODE_ID}
	}
